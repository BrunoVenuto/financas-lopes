...
import { TrendingUp, Plus, Sparkles, Loader2, Cloud } from "lucide-react";
...
const Dashboard: React.FC<{ onAction: (action: string) => void }> = ({ onAction }) => {
  ...
  const fetchAiInsight = async () => {
    ...
    try {
      ...
      const prompt = `Analise estas transações financeiras e dê um conselho curto (máximo 15 palavras) e motivador: ${transactions
        .slice(0, 5)
        .map((t) => `${t.description}: ${t.amount}`)
        .join(", ")}. Saldo atual: ${totalBalance}`;
  ...
  return (
    <div className="space-y-6 pb-24 animate-fade-up">
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              type="button"
            >
              <Cloud size={20} className="text-white/70" />
            </button>

            <button
              onClick={() => {
                console.log("[Dashboard] + clicado");
                onAction("new_transaction");
              }}
      ...
      <SectionTitle
        ...
        action={
          <button className="text-xs font-bold text-tanzine-300" onClick={() => onAction("view_transactions")} type="button">
            Ver Tudo
          </button>
        }
      />
        ...
        {recentTransactions.length > 0 ? (
          recentTransactions.map((tx) => {
            ...
            return (
              <GlassCard
                key={tx.id}
                className="!p-3 flex items-center gap-4 border-transparent hover:border-white/10 transition-all"
              >
                <div
